...
function addtoSuggestionContainerDOM(data) {
    ...
    // to check if the movie is already present in  the suggestion list array
    ...
    if (!isPresent && data.Title != undefined) {
        ...
        movieCard.setAttribute("class", "text-decoration");      //  doubt
...
//localstorage of addtofavorites
async function handleFavBtn(e) {
    const target = e.target;
    let data = await fetchMovies(target.dataset.id);   // doubt
    let favMoviesLocal = localStorage.getItem("favMoviesList");
    if (favMoviesLocal) {
        favMovieArray -= JSON.parse(favMoviesLocal);
    }
    else {
        localStorage.setItem("favMoviesList", JSON.stringigy(data));
    }
